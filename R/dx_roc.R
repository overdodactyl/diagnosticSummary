...
dx_roc <- function(dx_obj, curve_color = "red", text_color = "black", add_text = TRUE,
  ...
  citxt_v <- paste0("AUC: ", conf_int(cistring_v[2],cistring_v[1],cistring_v[3], accuracy = .001))

  sensdf <- dx_obj$data %>%
    dplyr::filter(!!as.name(dx_obj$options$true_varname) == 1) %>%
    dplyr::summarize(sens = mean(ifelse(!!as.name(dx_obj$options$pred_varname) <= dx_obj$options$setthreshold, 0, 1))) %>%
    as.vector()
  ...
  if (add_text){
    ...
    diagsummary_resultsdf <- subset(diagsummary_resultsdf, select = -c(Variable, Label))
