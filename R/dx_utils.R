...
summary.dx <- function(object, thresh = object$options$setthreshold,
  ...
  tmp <- as.data.frame(object, thresh = thresh, variable = variable, level = level)
...
#' @param ... additional arguments to be passed to or from methods
#' @export
as.data.frame.dx <- function(x, row.names = NULL, optional = TRUE, thresh = NA, variable = NA, level = NA, ...) {
  tmp <- x$measures
  if (!is.na(thresh)) {
