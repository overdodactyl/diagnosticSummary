...
dx_confusion_core <- function(predprob, truth, threshold, poslabel) {
  ...
  senscount <- pos %>% dplyr::group_by(testresult) %>% dplyr::summarize(counts=dplyr::n())
  speccount <- neg %>% dplyr::group_by(testresult) %>% dplyr::summarize(counts=dplyr::n())

  tp_1 <- dplyr::ungroup(senscount) %>% dplyr::filter(testresult ==1) %>% dplyr::select(counts) %>% as.vector()
  tp <- ifelse(length(tp_1[,1]) == 0, 0, tp_1[1,1]) %>% as.numeric()
  fn_1 <- dplyr::ungroup(senscount) %>% dplyr::filter(testresult ==0) %>% dplyr::select(counts) %>% as.vector()
  fn <- ifelse(length(fn_1[,1]) == 0, 0, fn_1[1,1]) %>% as.numeric()
  tn_1 <- dplyr::ungroup(speccount) %>% dplyr::filter(testresult ==0) %>% dplyr::select(counts) %>% as.vector()
...
dx_odds_ratio <- function(tp, tn, fp, fn) {
  if (tp == 0 | tn == 0 | fp == 0 | fn == 0) {
    ...
    sqrtsdlnor <- sqrt( 1/ (tp +0.5) + 1 / (tn+0.5) + 1/(fn+0.5) + 1/(fp+0.5))
